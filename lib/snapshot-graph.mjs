...
export function snapshotGraph(files) {
  ...
  // The token format has no concept of themes, so a snapshot cannot put them all on
  // one token's $value. Contract 2.1.0 carries them beside it instead, under
  // $extensions.modes, which means tokens.json alone is now a complete answer — and a
  // caller holding only that one file, a browser file picker being the obvious case,
  // can still show a token per theme.
  //
  // Older snapshots keep their themes only in the per-mode files, so those are still
  // read when the token does not carry them. Both paths fill the same two maps.
  ...
  for (const [tokenPath, t] of tokens) {
    if (t.ext?.figmaCollection) collectionOf.set(tokenPath, t.ext.figmaCollection);
    const modes = t.ext?.modes;
    if (modes && typeof modes === "object" && !Array.isArray(modes)) {
      modesOf.set(tokenPath, { ...modes });
    }
  }

  // Nothing left to read once every token carried its own themes. Reading the folder
  // anyway would be wasted work on the common path, and on the single-file path the
  // files simply are not there.
  const needsModeFiles = modesOf.size < tokens.size;
  let modeFilesRead = 0;
  ...
  if (needsModeFiles) {
    for (const file of manifest.files ?? []) {
      ...
      for (const [tokenPath, t] of collect(doc)) {
        ...
        if (!collectionOf.has(tokenPath)) collectionOf.set(tokenPath, file.collection);
      }
    }
